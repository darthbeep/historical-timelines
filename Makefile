...
lint:  ## run static analysis with flake8
	python -m black --check historical_timelines setup.py
	python -m flake8 historical_timelines setup.py

# Alias
lints: lint

format:  ## run autoformatting with black
	python -m black historical_timelines/ setup.py
...
annotate:  ## run type checking
	python -m mypy ./historical_timelines

#########
# TESTS #
#########
test: ## clean and run unit tests
	python -m pytest -v historical_timelines/tests

coverage:  ## clean and run unit tests with coverage
	python -m pytest -v historical_timelines/tests --cov=historical_timelines --cov-branch --cov-fail-under=75 --cov-report term-missing
